...
def evaluate(tokenizer, model, features, examples, dataset, language, prefix, eval_batch_size, model_type, out_dir,
             n_best_size, max_answer_length, version_2_with_negative, verbose_logging, do_lower_case, data_dir, task,
    ...
    if prefix == "test":
        if task == "tydiqa":
            data_file = os.path.join(data_dir, "tydiqa-goldp-v1.1-dev") + "/tydiqa." + language + ".test.json"
        else:
            data_file = os.path.join(data_dir, "MLQA_V1/test") + "/test-context-" + language + "-question-" + language \
                        + ".json"
    else:
        if task == "tydiqa":
            data_file = os.path.join(data_dir, "tydiqa-goldp-v1.1-train") + "/tydiqa." + language + ".train.json"
        else:
            data_file = os.path.join(data_dir, "SQUAD/squad") + "/dev-v1.1.json"
...
def run(n_gpu, stop_here, config_name, trans_model, model_type, tokenizer_name, do_lower_case, cache_dir, device,
        version_2_with_negative, null_score_diff_threshold, verbose_logging, data_dir, train_langs, dev_langs,
        test_langs, max_seq_length, doc_stride, max_query_length, pre_train_config, data_config, opt_config, out_dir,
        writer, freeze_bert, use_pretrained_model, pre_trained_model_name, use_adapt, task):
    ...
    print("Get train, dev and test Examples >>>>>")
    train_examples = processor.get_train_examples(data_dir, task, languages=train_langs)
                     # + processor.get_dev_examples(data_dir, task, languages=train_langs)

    few_shot_examples = processor.get_dev_examples(data_dir, task, languages=dev_langs)
                        # + processor.get_train_examples(data_dir, task, languages=dev_langs)

    dev_examples = processor.get_squad_dev_examples(data_dir, task)
    ...
    print("len(few_shot_examples):", len(few_shot_examples))
    ...
    print("Dev examples convertion to features")
    ...
    for lang in test_langs:
        test_examples.update({lang: processor.get_test_examples(data_dir, task, language=lang)})
        ...
        for _ in train_iterator:
            ...
            for step, batch in enumerate(epoch_iterator):
                ...
                if (step + 1) % pre_train_config["gradient_accumulation_steps"] == 0:
                    ...
                    """
                    dev_results = evaluate(tokenizer, model, dev_features, dev_examples, dev_dataset,
                                             ",".join(train_langs), "dev", pre_train_config["eval_batch_size"],
                                             model_type, out_dir, pre_train_config["n_best_size"],
                                             pre_train_config["max_answer_length"], version_2_with_negative,
                                             verbose_logging, do_lower_case, data_dir, task, null_score_diff_threshold, 
                                             lang2id)

                    average_score = 0.0
                    print("dev_results:", dev_results)
                    for key, value in dev_results.items():
                        writer.add_scalar("dev_{}".format(key), value, global_step)
                        average_score += value
    ...
    for lang in test_langs:
        test_results = evaluate(tokenizer, model, test_features[lang], test_examples[lang], test_dataset[lang],
                                ...
                                model_type, out_dir, pre_train_config["n_best_size"],
                                pre_train_config["max_answer_length"], version_2_with_negative,
                                verbose_logging, do_lower_case, data_dir, task, null_score_diff_threshold, lang2id)
    ...
    spt_sampler = RandomSampler(few_shot_dataset) #RandomSampler(train_dataset)
    spt_dataloader = DataLoader(train_dataset, sampler=spt_sampler, batch_size=data_config["k_spt"])

    t_total = len(spt_dataloader) // pre_train_config["gradient_accumulation_steps"] * num_train_epochs
    spt_scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps=pre_train_config["warmup_steps"],
                                                num_training_steps=t_total)

    ### Query

    few_shot_sampler = RandomSampler(few_shot_dataset) #RandomSampler(few_shot_dataset)
    qry_dataloader = DataLoader(few_shot_dataset, sampler=few_shot_sampler, batch_size=data_config["q_qry"])

    t_total = len(qry_dataloader) // pre_train_config["gradient_accumulation_steps"] * num_train_epochs
    qry_scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps=pre_train_config["warmup_steps"],
                                                    num_training_steps=t_total)

    ####
    #meta_learner = MetaLearner(opt_config, model, device, freeze_bert, use_adapt)
    #if n_gpu > 1:
    #    meta_learner = torch.nn.DataParallel(meta_learner)
    #meta_learner.cuda()
    ...
    for _ in tqdm(range(opt_config["epoch"])):
        epoch_iterator = tqdm(spt_dataloader, desc="Support Epoch Iteration", disable=local_rank not in [-1, 0])
        qry_epoch_iterator = tqdm(qry_dataloader, desc="Query Epoch Iteration", disable=local_rank not in [-1, 0])
        for step, (spt_batch, qry_batch) in enumerate(zip(epoch_iterator, qry_epoch_iterator)):

            meta_train_error, meta_tune_error = 0.0, 0.0
            n_tasks = 1

            #spt_batch = tuple(t.to(f'cuda:{model.device_ids[0]}') for t in spt_batch)
            spt_batch = tuple(t.to(device) for t in spt_batch)
            spt_inputs = {
                "input_ids": spt_batch[0],
                "attention_mask": spt_batch[1],
                "token_type_ids": None if model_type in ["xlm", "xlm-roberta", "distilbert"] else spt_batch[2],
                "start_positions": spt_batch[3],
                "end_positions": spt_batch[4],
            }

            if model_type in ["xlnet", "xlm"]:
                spt_inputs.update({"cls_index": spt_batch[5],
                                   "p_mask": spt_batch[6]})
                if version_2_with_negative:
                    spt_inputs.update({"is_impossible": spt_batch[7]})
            if model_type == "xlm":
                spt_inputs["langs"] = spt_batch[7]

            ##
            #qry_batch = tuple(t.to(f'cuda:{model.device_ids[0]}')for t in qry_batch)
            qry_batch = tuple(t.to(device) for t in qry_batch)
            qry_inputs = {
                "input_ids": qry_batch[0],
                "attention_mask": qry_batch[1],
                "token_type_ids": None if model_type in ["xlm", "xlm-roberta", "distilbert"] else qry_batch[2],
                "start_positions": qry_batch[3],
                "end_positions": qry_batch[4],
            }

            if model_type in ["xlnet", "xlm"]:
                qry_inputs.update({"cls_index": qry_batch[5], "p_mask": qry_batch[6]})
                if version_2_with_negative:
                    qry_inputs.update({"is_impossible": qry_batch[7]})
            if model_type == "xlm":
                qry_inputs["langs"] = qry_batch[7]
            ...
            maml = l2l.algorithms.MAML(model, lr=spt_scheduler.get_lr()[0], first_order=True)
            print("Initialized MAML")
            #maml = l2l.algorithms.LightningMAML(model, adaptation_lr=spt_scheduler.get_lr()[0], first_order=True)
            #maml = torch.nn.DataParallel(maml)
            for _ in tqdm(range(n_tasks), desc="Task Iteration"):
                learner = maml.clone()
                #learner.train()
                ...
                print("meta_train_error:", meta_train_error)
            ...
            if global_step % 20 == 0:
                for lang in test_langs:
                    test_results = evaluate(tokenizer, model, test_features[lang], test_examples[lang], test_dataset[lang],
                                            lang, "test", pre_train_config["eval_batch_size"], model_type, out_dir,
                                            pre_train_config["n_best_size"], pre_train_config["max_answer_length"],
                                            version_2_with_negative, verbose_logging, do_lower_case, data_dir, task,
                                            null_score_diff_threshold, lang2id)
...
def get_arguments():
    ...
    parser.add_argument('--batch-size', help="batch size in the pre-training process", type=int, default=4)
    parser.add_argument('--eval-batch-size', help="batch size in the pre-training process", type=int, default=4)
    ...
    parser.add_argument('--stop-here', help='Whether to do only pretraining', action='store_true')
...
if __name__ == "__main__":
    ...
    out_dir = os.path.join(args.out_dir, "MAML_QRY/"+args.trans_model+"/train_"+",".join(args.train_langs)+"-test_"
    ...
    print("AVAILABLE GPUS n_gpu:", n_gpu)

    #n_gpu = torch.cuda.device_count()

    run(n_gpu, args.stop_here, args.config_name, args.trans_model, args.model_type, args.tokenizer_name, args.do_lower_case,
        args.cache_dir, device, args.version_2_with_negative, args.null_score_diff_threshold, args.verbose_logging, args.data_dir,
        ...
        args.pre_trained_model_name, args.use_adapt, args.task)
