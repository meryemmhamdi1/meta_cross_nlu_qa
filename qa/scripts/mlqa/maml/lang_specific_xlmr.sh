...
echo "FEW-SHOT LEARNING ON EACH LANGUAGE INDEPENDENTLY"
for LANG in ar de es hi vi zh
do
    echo "FEW-SHOT ON "$LANG
    for SEED in 40 42 44 163
    do

        python main_new_data.py --use-slots --train-langs en --dev-langs $LANG --test-langs ar de es hi vi zh \
