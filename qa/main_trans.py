    ...
    for _ in tqdm(range(opt_config["epoch"])):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            for j in range(opt_config["n_task"]):
                ...
                if use_adapt:
                    learner = maml.clone()
                    for _ in range(opt_config["n_task"]):
                        for _ in range(0, opt_config["n_up_test_step"]):
                            tune_outputs = learner(**ada_inputs[])
...
if __name__ == "__main__":
    ...
    out_dir = os.path.join(args.out_dir, "MAML_TRANS_SIM_SEED"+str(args.seed)+"/train_"+",".join(args.train_langs)+"-test_"+",".join(args.test_langs)
