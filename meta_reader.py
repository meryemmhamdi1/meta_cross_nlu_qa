...
class MetaDataset(Dataset):
    ...
    def create_auxi_meta_train_batches(self):
        ...
        for _ in tqdm(range(self.batch_sz)):
            ...
            for i, input_ids in enumerate(inp_ids_qry_b):
                inp_ids_qry_b_pad.append(input_ids + [0]*(all_qry_set_batch[4] - len(input_ids)))
                tok_typ_ids_qry_b.append([1 for _ in range(all_qry_set_batch[4])])
                att_masks_qry_b.append([int(x > 0) for x in range(all_qry_set_batch[4])])
                slot_qry_pad.append(slot_l_qry_b[i] + [0]*(all_qry_set_batch[4] - len(slot_l_qry_b[i])))
    ...
    def next_batch(self, all_batches, batch_size, i):
        ...
        slot_l_spt_all, inp_ids_qry_all, tok_typ_ids_qry_all, att_masks_qry_all, \
