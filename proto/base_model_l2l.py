...
class PrototypicalTransformerNLU(nn.Module):
    def __init__(self, trans_model, num_intents, intent_types, slot_types, use_slots=False, num_slots=0,
        ...
        self.b_pad = nn.Parameter(torch.rand(1), requires_grad=True)
        self.b_O = nn.Parameter(torch.rand(1), requires_grad=True)
        self.b_X = nn.Parameter(torch.rand(1), requires_grad=True)

    def forward(self, input_ids_1, input_ids_2, intent_labels_1=None, intent_labels_2=None, slot_labels_1=None,
                      slot_labels_2=None, proto=None):
        """
        :param input_ids_1:
        :param input_ids_2:
        ...
        if self.use_slots:
            slot_proto = proto["slot"][0] #= self.encoder(proto["slot"][0])
        ...
        for m in range(0, len(decoded_2)):
            intent_sim = []
            for it, intent_prot in enumerate(intent_proto):
                cos_sim = self.cos(intent_prot, decoded_2[m][0])
                intent_sim.append(cos_sim)
    ...
    def test(self, input_ids, proto):
        ...
        lm_output_1 = lm_output_1[0] # average over all layers
