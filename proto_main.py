...
from proto.base_model import PrototypicalTransformerNLU
from proto.meta_learner_minimal import *
...
def compute_prototypes(dataset, data_size, data_part, model, use_slots,  plot_ext):
    ...
    for _ in tqdm(range(data_size)):
        ...
        model.trans_model.eval()
        with torch.no_grad():
            lm_output_1 = model.encoder(model.trans_model(input_ids)[0][0]) # average over all layers

        intent_proto_dict[true_intent].append(lm_output_1[0])
        X_i.append(lm_output_1[0].cpu().numpy().tolist())
        ...
        if use_slots:
            for j in range(1, len(lm_output_1)):
                ...
                X_s.append(lm_output_1[j].cpu().numpy().tolist())
    ...
    for intent in intent_proto_dict:
        ...
        if len(embed_list)>0:
            ...
            intent_proto_mean = torch.zeros(300).cuda()
        intent_mean_list.append(intent_proto_mean)
        X_i.append(intent_proto_mean.cpu().numpy().tolist())
    ...
    if use_slots:
        ...
        for slot in slot_proto_dict:
            ...
            if len(embed_list)>0:
                ...
                slot_proto_mean = torch.zeros(300).cuda()
            slot_mean_list.append(slot_proto_mean)
            X_s.append(slot_proto_mean.cpu().numpy().tolist())
...
def eval(dataset, meta_learner, test_langs, ext, proto):
    ...
    :return:
    """
    metrics = meta_learner.module.zero_shot_test(test_langs, dataset, ext, proto)

    print("metrics:", metrics)


def run(trans_model, use_pretrained_model, pre_trained_model_path, data_dir, train_langs, dev_langs, test_langs,
        use_few_shot, data_format, use_slots, data_config, opt_config, out_dir, use_aae, use_triplet_loss):
    """
    ...
    for epoch in tqdm(range(opt_config["epoch"])):
        ...
        for i in range(data_config["batch_sz"]//opt_config["n_task"]):
            ...
            for proto_type in prototypes:
                for pc in range(len(prototypes[proto_type])):
                    prototypes[proto_type][pc] = torch.cat(tuple(repeat(prototypes[proto_type][pc], len(device_ids))))

            meta_outputs = meta_learner(inp_ids_spt, tok_typ_ids_spt, att_masks_spt, len_spt, int_l_spt, slot_l_spt,
                                        inp_ids_spt_2, tok_typ_ids_spt_2, att_masks_spt_2, len_spt_2, int_l_spt_2,
                                        slot_l_spt_2, inp_ids_qry, tok_typ_ids_qry, att_masks_qry, len_qry, int_l_qry,
                                        slot_l_qry, prototypes)

            for proto_type in prototypes:
                for pc in range(len(prototypes[proto_type])):
                    meta_learner.module.proto[proto_type][pc] = meta_learner.module.proto[proto_type][pc].cuda()

            prototypes = meta_learner.module.proto

            if i % 10 == 0:
                # Testing
                print("Testing at Epoch {}, Step {} |")
                metrics = meta_learner.module.zero_shot_test(test_langs, dataset, ext="few_shot", proto=prototypes)
                ...
                for lang in test_langs:
                    ...
                    for metric in metrics[lang]:
                        ...
                        writer.add_scalar('META-Test-'+metric+"-"+lang, metrics[lang][metric], epoch*i)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--k-spt', help='Number of support examples per class', type=int, default=10)
    parser.add_argument('--q-qry', help='Number of query examples per class', type=int, default=5)
    ...
    dataset, model = run(args.trans_model, args.use_pretrained_model, args.pre_trained_model_name, args.data_dir,
                         args.train_langs, args.dev_langs, args.test_langs, args.use_few_shot, args.data_format,
                         args.use_slots, data_config, opt_config, args.out_dir, args.use_aae, args.use_triplet_loss)
