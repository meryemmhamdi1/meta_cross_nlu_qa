...
def nlu_evaluation(model, dataset, lang, nb_examples, use_slots, device):
    model.eval()
    ...
    for _ in range(nb_examples):
        ...
        if use_slots:
            intent_logits, slot_logits, intent_loss, slot_loss = model(input_ids=input_ids, intent_labels=intent_labels,
            ...
            intent_logits, intent_loss = model(input_ids=input_ids, intent_labels=intent_labels)
...
def pre_train_from_scratch(model, device, dataset, dataset_type,  pre_train_config, use_slots, test_langs, writer, out_dir):
    ...
    for i in range(pre_train_config["pre_train_steps"]):
        ...
        batch, _ = dataset.next_batch(pre_train_config["batch_size"], dataset_type)
        ...
        if i > 0 and i % 100 == 0:
            ...
            if test_intent_f1_avg > best_test_intent_f1:
                print("Saving the best model based on cross-lingual validation intent accuracy to ...", output_model_file)
                no_impr = 0
